...
func GetRisks(ctx *gin.Context) {
	defer func() {
		if err := recover(); err != nil {
			ctx.JSON(http.StatusInternalServerError, constants.ERR_MSG_INTERNAL_SERVER_ERROR)
			return
		}
	}()
...
func GetRisk(ctx *gin.Context) {
	...
	if _, err := uuid.Parse(idFromPath); err != nil {
		ctx.JSON(http.StatusBadRequest, constants.ERR_MSG_INVALID_RISK_ID)
	...
	if risk == nil {
		ctx.JSON(http.StatusNotFound, constants.ERR_MSG_RISK_NOT_FOUND)
